...
def build_mlp_graph(input_tensor, out_size, n_hidden, hidden_activation=tf.nn.relu, output_activation=None, use_dropout=False, use_bn=False, bn_is_training=False):
    ...
    # kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_IN', uniform=False) , #ANIMA
    # kernel_initializer=tf.initializers.random_normal(stddev=0.1), #ANIMA
    for ind,n in enumerate(n_hidden):
        ...
        # current_input = tf.contrib.layers.layer_norm(current_input)  #ANIMA
