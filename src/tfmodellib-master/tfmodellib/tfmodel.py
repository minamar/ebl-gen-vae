...
def docsig(f):
    ...
    if 'signature' in dir(inspect):
        ...
        # python2
        sig = inspect.formatargspec(*inspect.signature(f))
...
class TFModel(object):
    ...
    def init_summaries(self):
        if self.config['summaries_root'] is not None:
            ...
            #ANIMA
            self.add_summary(tag='({:s}) epoch training reconstruction loss'.format(self.config['scope_name']), key='reconstruction_loss', mode='epoch')
            self.add_summary(tag='({:s}) epoch training variational loss'.format(self.config['scope_name']), key='variational_loss', mode='epoch')
    ...
    def train(self, train_inputs=None, train_targets=None, validation_inputs=None, validation_targets=None, batch_size=None, *args, **kwargs):
        ...
        training_loss, validation_loss, reconstruction_loss, variational_loss = result  #ANIMA
        ...
        if self.config['epoch_summaries_interval'] is not None \
            ...
            self.update_summary('reconstruction_loss', reconstruction_loss)     #ANIMA
            self.update_summary('variational_loss', variational_loss)       #ANIMA
        ...
        return training_loss, validation_loss, reconstruction_loss, variational_loss
        ...
        if inds is not None:
            ...
            with self.graph.as_default():
                with tf.control_dependencies(self.graph.get_collection(tf.GraphKeys.UPDATE_OPS)):
                    training_loss, reconstruction_loss, variational_loss = self.run_update_and_loss(
            ...
            with self.graph.as_default():
                with tf.control_dependencies(self.graph.get_collection(tf.GraphKeys.UPDATE_OPS)):
                    training_loss, reconstruction_loss, variational_loss = self.run_update_and_loss(*args, **kwargs)  #ANIMA
