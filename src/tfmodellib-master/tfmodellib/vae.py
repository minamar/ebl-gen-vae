...
@docsig
def build_vae_graph(
        input_tensor, latent_size, encoder_size, decoder_size=None, #labels_tensor,
    ...
    with tf.variable_scope(encoder_name):
        ...
        # encoder_out = tf.contrib.layers.layer_norm(encoder_out)  #ANIMA
...
if __name__ == '__main__':
    ...
    xx, yy, zz = np.random.rand(3 * 10000).reshape((3, -1))
    ...
    latent_u, latent_v, latent_w = latent_mean[:, dim_inds].transpose()
    ax = fig.add_subplot(1,3,2,projection='3d')
    # for ind in range(l.size):
    #     ax.plot(latent_u[ind], latent_v[ind], latent_w[ind], ls='-', marker='None', color='b')
    #     ax.plot(latent_u.T[ind], latent_v.T[ind], latent_w.T[ind], ls='-', marker='None', color='g')

    ax.plot(latent_u, latent_v, latent_w, ls='none', marker='.',mec='none', color='b')
    # ax.scatter(latent_u.T, latent_v.T, latent_w.T, ls='-', marker='None', color='g')
