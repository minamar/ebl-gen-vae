...
z_dataset = check_model + check_epoch + '_df13_50fps_mean.csv'
...
# Load reconstructed
df_reco = pd.read_csv(os.path.join(ROOT_PATH, DATA_RECO, check_model + check_epoch, reco_dataset), index_col=0)
...
for anim_id in anim_id_list:
    ...
    fig = x2reco(df_x_anim, df_z_mean_anim, df_reco_anim)

    if save_stuff:
        plot_path = os.path.join(ROOT_PATH, DATA_VISU, 'reconstruction', check_model + check_epoch)
