...
df_z_mean_new.iloc[:, 0:z_dim] = df_z_mean.iloc[:, 0:z_dim].add(df_z_sigma.iloc[:, 0:z_dim].multiply(eps))
...
for anim_id in anim_id_list:
    ...
    fig = z_anim2dec(df_z_mean_new, interp, output_df, anim_id, category)

    if save_stuff:
        # Graphs
        plot_path = os.path.join(ROOT_PATH, DATA_VISU, 'interp_nsteps_anim_eps', check_model + check_epoch)
        ...
        df_path = os.path.join(ROOT_PATH, DATA_SAMP, 'interp_nsteps_anim_eps', check_model + check_epoch)
